  ...
  export default {
    ...
    methods: {
      ...
      async sendCode () {
        ...
        // 发请求 ==> 发短信的接口
        const result = await this.$API.user.sendCode({phone: this.phone})
        if (result.code===0) {
          Toast('短信发送成功!')
        } else {
          // 停止计时
      ...
      */
      async login () {
        ...
        if (success) {
          ...
          let result
          if (this.loginWay) {
            result = await this.$API.user.loginSms({phone, code})
            // 请求结束后, 停止计时
            this.computeTime = 0
          } else {
            result = await this.$API.user.loginPwd({name, pwd, captcha})
